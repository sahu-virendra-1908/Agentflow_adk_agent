...
// Basic fetch typing so TS doesn't complain in Node environment
// (runtime pe Node 18+ / 20+ already fetch provide karta hai)
...
export const backtestStrategyTool = createTool({
  name: "backtest_strategy",
  description:
    "Backtest a simple spot strategy on historical daily prices. Use when the user asks to backtest or simulate a rule like 'buy ETH when it drops 5%.",
  schema: z.object({
    asset: z
      .string()
    ...
    dropPercent: z
      ...
      .default(5)
      .describe(
        "Percent drop from previous close that triggers a buy. E.g. 5 = buy if today's close is 5% below yesterday."
      ),
    initialCapital: z
      .number()
      .min(100)
  ...
  fn: async (args) => {
    ...
    if (!res.ok) {
      throw new Error(
        `Failed to fetch price history for ${asset} from CoinGecko: ${res.status}`
      );
    }
    ...
    for (let i = 1; i < prices.length - 1; i++) {
      ...
      const drop =
        ((prev.price - today.price) / prev.price) * 100; // % drop from prev
      ...
      if (position === 0 && drop >= dropPercent) {
        ...
        const pnlPercent =
          ((sellValue - initialCapital) / initialCapital) * 100;
    ...
    const summary = `In the last ${lookbackDays} days, starting with $${initialCapital.toFixed(
      2
    )}, this simple "${dropPercent}% drop then buy, sell next day" strategy on ${asset.toUpperCase()} would result in approximately ${totalReturnPct.toFixed(
      2
    )}% total return across ${trades.length} trades.`;
